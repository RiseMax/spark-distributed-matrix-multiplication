...
1.测试不同规模$1000\times 1000$，$2000\times 2000$，$5000\times 5000$方阵的乘法，稀疏度5%

2.测试$2000\times 2000$的方阵在不同稀疏度0.1%，1%，5%，50%，100%下的乘法

3.测试行多的矩阵乘法，$A$矩阵$5000\times 500$，$B$矩阵$500\times 2000$ 

4.测试列多的矩阵乘法，$A$矩阵$500\times 3000$，$B$矩阵$3000\times 5000$

5.测试矩阵和向量的乘法，$A$矩阵$2000\times 1000$，$B$向量$1000\times 1$

6.测试向量和矩阵的乘法，$A$向量$1\times 1000$，$B$矩阵$1000\times 2000$

7.测试特殊矩阵， $A$矩阵$3000\times 2000$，瘦右矩阵$B_1$ $2000\times 40$和不是瘦右矩阵$B_2$ $2000\times 80$，转置矩阵$A\times A^T$ 

#### 指标

运行时间，$shuffle$数据量，$cpu$利用率

记录的指标$matrices,multiply,duration,task time,shuffle read,shuffle write$

1与2数据集我们的四种方法都测，分别绘制这三种指标的折线图

然后选择我们自己实现的三种方法中表现最好的，和systemds的实现对比，跑3、4、5、6、7数据集，只对比时间，预期systemds的实现更快，根据源码分析原因
...
2、安全组设置云服务器$\to$安全组$\to$管理规则$\to$增加规则
