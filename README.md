...
四、SpGEMM
优点
稀疏场景针对性强：作为稀疏矩阵乘法的专用算法，在低稀疏度（如图 1 的 5%）下，耗时仅略高于 Broadcast（5000×5000 时为 302 秒）；在高稀疏度场景（如图 2 的 100%）下，性能优势更明显。
资源消耗低：CPU 使用率、Shuffle 数据量与 Broadcast 处于同一梯队，是 “稀疏矩阵 + 分布式” 场景的高效选择。
缺点
